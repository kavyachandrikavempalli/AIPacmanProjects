# -*- coding: utf-8 -*-
"""
Created on Thu Feb 22 22:57:43 2024
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minmax(agent,gameState,depth):
            ...
            if agent==0:
                ...
                for action in actionsAllowed:
                    ...
                    val=minmax(agent+1,successorState,depth+1)
                    optimalScore=max(optimalScore,val)
                    bestAction=action
                return optimalScore
            else:
                optimalScore=float('inf')
                ...
                for actions in actionsAllowed:
                    successorState=gameState.generateSuccessor(1,action)
                    val=minmax(agent+1,successorState,depth+1)
                    optimalScore=min(optimalScore,val)
                    bestAction=action
                return optimalScore
        actionsAllowed=gameState.getLegalActions(0)
        optimalAction=None
        optimalScore=float('-inf')
        # print(actionsAllowed)
        for action in actionsAllowed:
            # print(action,optimalScore)
            successorState=gameState.generateSuccessor(0,action)
            val=minmax(0,successorState,depth)
            if val>optimalScore:
                optimalScore=val
